...
function show(startDate, endDate, itemId){
    ...
    $.getJSON(apiURL+"/purchase/read.php?type=purchaseReport"+params+"&ts="+Math.random(), function(data){  // Change Needed HERE
    ...
    read_html+="</div>";
        ...
        $.each(data.purchase, function(key, val) { 
            ...
            if(val.itemName != currentItem && currentItem != ""){
                ...
                read_html+="<td colspan=3 class='text-right text-danger'>Total</td><td>"+parseFloat(totalItemPurchase).toFixed(2)+"</td>";
                read_html+="</table>";
                read_html+="</div>";
