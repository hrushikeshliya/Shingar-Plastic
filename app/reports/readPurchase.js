...
function show(startDate, endDate, itemId) {
  ...
  $.getJSON(
    apiURL +
      "/purchase/read.php?type=purchaseReport" +
      params +
      "&ts=" +
      Math.random(),
    function (data) {
      ...
      read_html +=
        ...
        "' max='" +
        $.cookie("endDate") +
        "' class='form-control pull-left m-b-15px'/>";
      read_html += "</div>";
      ...
      read_html += "Item Name :";
      read_html +=
        "<select id='itemId' name='itemId' class='form-control pull-left m-b-15px'>";
      read_html += "<option></option>";

      $.getJSON(
        apiURL + "/item/read.php" + "?ts=" + Math.random(),
        function (data4) {
          $.each(data4.item, function (key4, val4) {
            if (itemId == val4.id) {
              ...
              read_html +=
                "<option value=" + val4.id + ">" + val4.name + "</option>";
            }
          });
          ...
          read_html +=
            "<div id='search' class='btn btn-success pull-left m-b-15px search-button'>";
          ...
          $.each(data.purchase, function (key, val) {
            ...
            if (val.itemName != currentItem && currentItem != "") {
              ...
              read_html +=
                "<td colspan=2 class='text-right text-danger'>Total</td><td>" +
                parseFloat(totalItemQuantity).toFixed(2) +
                "</td><td>" +
                parseFloat(totalItemPurchase).toFixed(2) +
                "</td>";
              read_html += "</table>";
              read_html += "</div>";
            ...
            totalItemQuantity += +val.quantity;
          ...
          read_html +=
            "<h5 class='text-danger m-l-15px'>Total Quantity : <span id='totalQty'></span> &nbsp;&nbsp;&nbsp;Total Amount : <span id='totalAmt'></span></h5>";
          read_html += "<HR>";
