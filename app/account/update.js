$(document).ready(function () {
	...
	$.getJSON(apiURL + "/city/readCity.php?ts=" + Math.random(), function (data) {
		$.each(data.records, function (key, val) {
			city_options_html += "<option value='" + val.cityName + "'>" + val.cityName + "</option>";
		});
	});

	$.getJSON(apiURL + "/city/readState.php?ts=" + Math.random(), function (data) {
		$.each(data.records, function (key, val) {
			state_options_html += "<option value='" + val.stateName + "'>" + val.stateName + "</option>";
		});
	});

	$.getJSON(apiURL + "/accountType/read.php?ts=" + Math.random(), function (data) {
		$.each(data.accountType, function (key, val) {
			accountType_options_html += "<option value='" + val.Id + "'>" + val.name + "</option>";
		});
	});

	$.getJSON(apiURL + "/transport/read.php?ts=" + Math.random(), function (data) {
	...
	$(document).on('click', '.update-button', function () {
		...
		$.getJSON(apiURL + "/account/readOne.php?id=" + id + "&ts=" + Math.random(), function (data) {   // Change Needed HERE
			...
			update_html += "<td><input value=\"" + Math.abs(data.openingBalance) + "\" type='number' step='0.001' name='openingBalance' class='form-control col-md-6' min='0' required />" + openingDirection_options_html + "</td>";
